...
{
    /**
     * Handle agent login with email/username and password.
     */
    public function login(Request $request): JsonResponse
    {
        ...
        if ($user->hasEnabledTwoFactorAuthentication()) {
            ...
            return response()->json([
                ...
                'two_factor' => true,
                'redirect' => route('agent.two-factor.challenge'),
            ]);
        }
        ...
        return response()->json([
            ...
            'redirect' => route('agent.transactions'),
        ]);
    }
    ...
    public function twoFactorChallenge(Request $request): JsonResponse
    {
        ...
        $loginRole = $request->session()->get('login.role');

        if (! $userId || $loginRole !== 'agent') {
            throw ValidationException::withMessages([
        ...
        if (! $user->isAgent()) {
            $request->session()->forget(['login.id', 'login.remember', 'login.role']);
            throw ValidationException::withMessages([
